<?php
	...
	if(empty($_REQUEST['cl']) || empty($_REQUEST['f']))
	{
		...
		$query = mysqli_query($conection,"SELECT f.nofactura, DATE_FORMAT(f.fecha, '%d/%m/%Y') as fecha,  DATE_FORMAT(f.fecha,'%H:%i:%s') as  hora, f.codcliente, f.estatus,
												 ...
												 cl.doc, cl.nombre, cl.telefono,cl.direccion
											FROM factura f
		...
		if($result > 0){
			...
			ob_start(); // guarda o carga en memoria el siguiente archivo "factura.php"
		  include(dirname('__FILE__').'/factura.php'); // con dirname accedemos a la ruta donde se encuentra el archivo
		  $html = ob_get_clean();

// Este codigo es el que nos permite el uso de la libreria dompPDF para generar un archivo PDF
